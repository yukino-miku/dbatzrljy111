...
_FIGURE_CONTEXT = "问题二 Standard 二重覆盖星座（M=40，N=46，S=1840）"


def set_problem3_figure_context(selected) -> None:
    global _FIGURE_CONTEXT
    _FIGURE_CONTEXT = (
        "问题二 Standard 二重覆盖星座"
...
def plot_topology_3d_snapshot(
    ...
    ax.set(
        ...
        title=_title(f"问题三星间链路三维拓扑快照（t={snapshot.time_seconds / 60:.1f} min）"),
    )
...
def plot_same_plane_distance(
    ...
    ax.set(xlabel="时间 / min", ylabel="同轨相邻卫星距离 / km", title=_title("同轨相邻链路距离的解析与数值校核"))
    ax.legend()
...
def plot_cross_plane_distance_time(link_timeseries: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set(xlabel="时间 / min", ylabel="跨轨链路距离 / km", title=_title("代表性跨轨链路距离随时间变化"))
    ax.legend()
...
def plot_network_connectivity_time(topology: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig.suptitle(_title("星间网络连通性随时间变化"), y=0.995)
...
def plot_link_availability_distribution(availability: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.set(xlabel="链路可用率", ylabel="候选链路数量", title=_title("星间链路可用率分布"))
    ax.legend()
...
def plot_crosslink_switches(shifts: pd.DataFrame, output_dir: Path) -> None:
    ...
    axes[0].set(xlabel="时间 / min", ylabel="循环偏移 κ", title=_title("代表性相邻轨道面对的循环移位"))
    ...
    axes[1].set(xlabel="左侧轨道面编号", ylabel="偏移切换次数", title=_title("各相邻轨道面对的匹配切换次数"))
...
def plot_example_route_3d(
    ...
    ax.set(xlim=(-limit, limit), ylim=(-limit, limit), zlim=(-limit, limit), xlabel="ECEF x / km", ylabel="ECEF y / km", zlabel="ECEF z / km", title=_title("最大时延样本的端到端最小时延路径"))
    ax.set_box_aspect((1, 1, 1))
...
def plot_example_route_map(worst: dict[str, object], positions_ecef: np.ndarray, output_dir: Path) -> None:
    ...
    ax.set(xlabel="经度 / °E", ylabel="纬度 / °N", title=_title("最大时延样本路径的地面投影"))
...
def plot_routing_figures(
    ...
    ax.set(xlabel="端到端总时延 / ms", ylabel="累计概率", title=_title("端到端最小时延累计分布"))
    ax.legend()
    ...
    ax.set(xlabel="仿真时间 / h", ylabel="时延 / ms", title=_title("区域点对时延随时间变化"))
    ax.legend()
    ...
    ax.set(ylabel="平均时延 / ms", title=_title("端到端时延构成"))
    ...
    ax.set(xlabel="地面点大圆距离 / km", ylabel="端到端总时延 / ms", title=_title("地面距离与端到端时延关系"))
    ...
    if not heatmap.empty:
        ...
        ax.set(xlabel="目的地经度 / °E", ylabel="目的地纬度 / °N", title=_title("选定源点到区域网格的最小时延"))
        colorbar = fig.colorbar(scatter, ax=ax)
...
def plot_traffic_figures(
    ...
    _style()
    fig, ax = plt.subplots()
    ax.plot(hours, traffic["actual_region_demand_gbps"] / 1000.0, label="目标区域总流量")
    ax.plot(hours, traffic["satellite_demand_gbps"] / 1000.0, label="进入卫星网络的需求")
    ax.set(xlabel="时间 / h", ylabel="流量 / Tbps", title=_title("目标区域流量与卫星承载流量曲线"))
    ax.legend()
    _save(fig, output_dir / "fig_problem3_traffic_profile.png")

    _style()
    fig, ax = plt.subplots()
    ax.plot(hours, traffic["satellite_demand_gbps"] / 1000.0, label="需求")
    ax.plot(hours, traffic["optimized_throughput_gbps"] / 1000.0, label="成功吞吐量")
    ax.plot(hours, traffic["optimized_blocked_gbps"] / 1000.0, label="阻塞流量")
    ax.set(xlabel="时间 / h", ylabel="流量 / Tbps", title=_title("需求、吞吐量与阻塞流量"))
    ax.legend()
    ...
    ax.set(xlabel="时间 / h", ylabel="公平服务比例 r(t)", ylim=(0, 1.03), title=_title("公平服务比例随时间变化"))
    ...
    if not satellite_loads.empty:
        ...
        ax.set(xlabel="卫星编号", ylabel="时间快照序号", title=_title("时间—卫星接入利用率热力图"))
    ...
    ax.set(xlabel="时间 / h", ylabel="接入容量利用率", title=_title("卫星最大与平均利用率"))
    ax.legend()
    ...
    if not comparison.empty:
        ...
        labels = ["平均吞吐量 / Tbps", "最大利用率", "平均接入代价 / ms"]
        fig, axes = plt.subplots(1, 3, figsize=(11.0, 4.2))
        for axis, metric, label in zip(axes, metrics, labels):
            values = comparison[metric] / 1000.0 if metric == "mean_throughput_gbps" else comparison[metric]
            axis.bar(comparison["strategy_cn"], values, color=["#8d99ae", "#2a9d8f"])
            axis.set_title(label, fontsize=10)
        fig.suptitle(_title("最近卫星基准与优化接入方案对比"), y=1.02)
    ...
    if not sensitivity.empty:
        ...
        ax1.set(xlabel="卫星业务承载比例 η_sat / %", ylabel="服务满足率", title=_title("卫星承载比例敏感性"))
