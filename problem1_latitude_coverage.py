...
def _format_inclination_ranges(values: list[float], step: float = INCLINATION_STEP_DEG) -> str:
    ...
    for value in values[1:]:
        if abs(value - prev - step) <= 1e-9:
            ...
            start = prev = value
    ranges.append((start, prev))
    chunks = [f"{lo:.2f}°" if lo == hi else f"{lo:.2f}-{hi:.2f}°" for lo, hi in ranges]
    return ", ".join(chunks)
...
本次主仿真采用 `theta_given`，原因是题目明确给出 Ku 波段半锥角对应的地面覆盖半径约 506 km。由 alpha 反推的 theta 作为一致性校核列入表格。根据当前参数，`theta_alpha = {theta_alpha_deg:.6f}°`，对应地面弧长 `{summary['ground_radius_from_alpha_km']:.3f} km`；`theta_given = {theta_deg:.6f}°`。采用题目给定覆盖半径时，球冠面积为 `{summary['spherical_cap_area_km2']:.3f} km^2`，局部平面近似面积为 `{summary['flat_area_km2']:.3f} km^2`，二者相对差为 `{summary['area_relative_error']:.6%}`。
...
程序对 i=40° 到 60°、步长 0.25° 的倾角序列进行扫描。对每个倾角，从 N=1 到 80 递增枚举。每一个 (i,N) 组合都在一个轨道周期内采样 `{NUM_TIME_SAMPLES}` 个时刻，计算 C_lat(t) 的最小值。若第一个 N 满足
...
若 i+theta<50°，即使同一轨道面内卫星数量无限增多，星下点最高纬度加覆盖地心角仍无法达到目标纬度带上边界 50°N，因此该倾角在几何上不可行。当前采用 theta_given 时，因上边界条件失败而不可行的倾角范围为：{infeasible_upper_text}。但这个条件只是必要条件，最终是否连续覆盖仍以时间采样区间并集结果为准。在几何上通过上界判断但 N<=80 仍未找到连续覆盖方案的倾角范围为：{no_n_found_text}。

程序确实扫描了 40° 到 60° 的完整倾角范围。40° 没有 N_min 不是因为程序漏算，而是因为当前模型要求整个 30°N 到 50°N 纬度带在纬度投影方向连续覆盖。对 40° 轨道，最高可覆盖纬度为 i+theta={upper_reach_40:.6f}°N，达不到 50°N 上边界；即使取 N=80，完整扫描得到的 min_coverage 仍为 {float(row_40_80['min_coverage']):.6f}。若采用“纬度带内任意一点可见”的弱判据，40° 的结论会不同；本文采用的是更强的纬度带全覆盖判据，目的是为后续区域覆盖优化提供保守基础。
...
当前参数下，主仿真采用的覆盖地心角为 `{theta_deg:.6f}°`，地面覆盖半径为 `{summary['ground_radius_given_km']:.3f} km`，球冠覆盖面积为 `{summary['spherical_cap_area_km2']:.3f} km^2`。在所有扫描倾角中，最小的 N_min 为 `{best_n_text}`，对应倾角范围为：{best_i_text}。
