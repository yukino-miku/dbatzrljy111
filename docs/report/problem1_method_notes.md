...
本次主仿真采用 `theta_given`，原因是题目明确给出 Ku 波段半锥角对应的地面覆盖半径约 506 km。由 alpha 反推的 theta 作为一致性校核列入表格。根据当前参数，`theta_alpha = 4.364469 deg`，对应地面弧长 `485.307 km`；`theta_given = 4.550567 deg`。采用题目给定覆盖半径时，球冠面积为 `803938.086 km^2`，局部平面近似面积为 `804360.817 km^2`，二者相对差为 `0.052583%`。
...
程序对 i=40 deg 到 60 deg、步长 0.25 deg 的倾角序列进行扫描。对每个倾角，从 N=1 到 80 递增枚举。每一个 (i,N) 组合都在一个轨道周期内采样 `3000` 个时刻，计算 C_lat(t) 的最小值。若第一个 N 满足
...
若 i+theta<50 deg，即使同一轨道面内卫星数量无限增多，星下点最高纬度加覆盖地心角仍无法达到目标纬度带上边界 50 deg，因此该倾角在几何上不可行。当前采用 theta_given 时，因上边界条件失败而不可行的倾角范围为：40.00-45.25 deg。但这个条件只是必要条件，最终是否连续覆盖仍以时间采样区间并集结果为准。在几何上通过上界判断但 N<=80 仍未找到连续覆盖方案的倾角范围为：无。

程序确实扫描了 40 deg 到 60 deg 的完整倾角范围。40 deg 没有 N_min 不是因为程序漏算，而是因为当前模型要求整个 30 deg N 到 50 deg N 纬度带在纬度投影方向连续覆盖。对 40 deg 轨道，最高可覆盖纬度为 i+theta=44.550567 deg N，达不到 50 deg N 上边界；即使取 N=80，完整扫描得到的 min_coverage 仍为 0.725725。若采用“纬度带内任意一点可见”的弱判据，40 deg 的结论会不同；本文采用的是更强的纬度带全覆盖判据，目的是为后续区域覆盖优化提供保守基础。
...
当前参数下，主仿真采用的覆盖地心角为 `4.550567 deg`，地面覆盖半径为 `506.000 km`，球冠覆盖面积为 `803938.086 km^2`。在所有扫描倾角中，最小的 N_min 为 `23`，对应倾角范围为：46.00-46.75 deg。
