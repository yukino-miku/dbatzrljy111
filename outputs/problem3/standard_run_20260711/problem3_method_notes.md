...
> **星座输入限制：** 所选 Problem 2 Standard 候选为二重覆盖 40×46 星座，但其记录的验证状态为 `standard_search_passed_requires_full`。Problem 3 固定继承该输入，不把它重新解释为已通过 Full 验证。
...
程序实际读取 `outputs/problem2/standard_M25-50_N35-50/best_double_constellation.json`，SHA-256 为 `db7f9240ea3b19cf980bb294cfaf7bfdc02afa6dae9d13dae9303edce5f0f7e3`，星座签名为 `standard_double_M40_N46_S1840`。该文件是 Problem 2 Standard 二重覆盖结果。星座参数为：轨道面数 $M=40$，每面卫星数 $N=46$，总卫星数 1840；倾角 $i=50.33134363^\circ$，相位因子 $F=28$，初始升交点 $\Omega_0=3.59413675^\circ$，初始纬度辐角 $u_0=4.29568373^\circ$，高度 $h=550.0$ km，覆盖地心角 $\theta=4.55056733^\circ$。轨道面升交点按 $\Omega_p=\Omega_0+360^\circ p/M$ 均匀布置，面内初相位与相邻面相位差继续使用 Problem 2 的 Walker 规则。

问题三固定继承这些参数，因为题意研究的是给定星座的链路和路由。若再次调整 $M,N,i,F,\Omega_0,u_0$，就会把问题三错误地变成新的星座设计问题。

## 3. 时变图与星间拓扑

在时刻 $t$ 建立无向图 $G(t)=(V,E(t))$。节点 $V$ 是全部 1840 颗卫星，节点编号统一为 $s=pN+q$。边 $E(t)$ 是当时同时满足通信距离和地球视线条件的链路。卫星位置直接复用 Problem 2 的圆轨道二体传播：$u_{pq}(t)=u_0+2\pi q/N+2\pi Fp/(MN)+nt$，$n=\sqrt{\mu/(R+h)^3}$，再由倾角和升交点旋转得到 ECI 三维坐标。
...
20 Gbps 是题目明确给出的单星接入容量。1840 颗卫星对应绝对理论接入容量上界 36.8 Tbps，只相当于平均需求的 16.37% 和峰值需求的 10.91%；实际可服务比例还会受当时覆盖目标区域的卫星数量限制。若连 20 Gbps 约束也忽略，吞吐量恒等于需求，第三小问将失去拥塞控制含义。题目没有给出每条激光 ISL 的容量，因此主模型不编造链路带宽，只让 ISL 影响路径是否存在和端到端时延。该简化不能判断某条 ISL 是否发生带宽拥塞；获得链路容量后可扩展为带容量的多商品流模型。

## 9. 实际区域流量数据处理

目标区域严格取 $4^\circ\mathrm{N}\sim53^\circ\mathrm{N}$、$73^\circ\mathrm{E}\sim135^\circ\mathrm{E}$ 的完整球面经纬度矩形，不使用中国行政国土面积。球面面积为

$$A_{region}=R^2\Delta\lambda\left(\sin\varphi_{max}-\sin\varphi_{min}\right)=32013984.2302\ \mathrm{km}^2.$$

用户给定统一流量密度 $7.02139\ \mathrm{Mbit/(s\cdot km^2)}$，它等价于 $7.02139\ \mathrm{Mbps/km^2}$，已经是速率，不能再除以秒、小时或统计周期。因此 $D_{mean}=224.782669$ Tbps，$D_{peak}=1.5D_{mean}=337.174003$ Tbps，日变化最低值为 112.391334 Tbps。每个规则网格按照 $w_g\propto\cos\varphi_g$ 表示的球面面积权重分配，$A_g=w_gA_{region}$，并在程序中检查 $\sum_gA_g=A_{region}$、$\sum_gd_g=D(t)$。当前数据来源名称：用户给定；来源链接：未提供。主结果使用 $\eta_{sat}=1$，不自动缩放为 0.1%、1%、5% 或 10%。
...
拓扑结果：同轨相邻距离解析值为 944.611 km，跨轨活动链路距离范围为 410.409 km 至 931.280 km。网络全连通时间比例为 1.0000，观测到的最大节点度为 4，拓扑切换累计 0 次。

路由结果：可达请求平均时延为 19.6125 ms，95% 分位时延为 34.9599 ms，最大时延为 48.2951 ms，不超过 30 ms 的比例为 0.8753。平均 ISL 跳数为 4.473，最大跳数为 15。

流量结果：用户给定密度对应区域平均需求 224.782669 Tbps，理论峰值需求 337.174003 Tbps。本次离散时段内优化方案平均吞吐量为 2.633888 Tbps，平均服务比例为 0.013530，峰值需求时服务比例为 0.007857，平均阻塞量为 222.148781 Tbps，峰值需求时阻塞量为 334.524765 Tbps，最大卫星利用率为 1.0000。
...
**问题分析段。** 在问题二确定区域覆盖星座后，问题三需要进一步判断该星座能否形成稳定的星间通信网络，并研究区域通信请求的最小时延路由以及高并发业务下的接入分流。为避免改变前问结论，本文固定采用 Problem 2 Standard 输出的 40×46 Walker 星座。
...
**拓扑结果段。** 同轨相邻距离解析值为 944.611 km，跨轨活动链路距离范围为 410.409 km 至 931.280 km。网络全连通时间比例为 1.0000，观测到的最大节点度为 4，拓扑切换累计 0 次。

**路由结果段。** 可达请求平均时延为 19.6125 ms，95% 分位时延为 34.9599 ms，最大时延为 48.2951 ms，不超过 30 ms 的比例为 0.8753。平均 ISL 跳数为 4.473，最大跳数为 15。

**流量优化结果段。** 用户给定密度对应区域平均需求 224.782669 Tbps，理论峰值需求 337.174003 Tbps。本次离散时段内优化方案平均吞吐量为 2.633888 Tbps，平均服务比例为 0.013530，峰值需求时服务比例为 0.007857，平均阻塞量为 222.148781 Tbps，峰值需求时阻塞量为 334.524765 Tbps，最大卫星利用率为 1.0000。
