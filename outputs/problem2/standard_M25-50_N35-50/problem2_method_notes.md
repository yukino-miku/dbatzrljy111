...
> 本文件由 `problem2_constellation_ga.py` 根据实际 JSON 与 CSV 结果生成。当前运行模式为 `standard`。遗传算法只能说明在给定搜索范围、仿真时段、空间网格和多随机种子条件下找到的最优已搜索方案，不能证明数学意义上的全局最优。`quick` 结果只用于程序冒烟测试，不得直接作为论文最终结论。
...
搜索范围为 M∈[25,50]、N∈[35,50]、i∈[40.0°,60.0°]、F∈{0,…,M-1}。几何必要条件 i+θ≥53° 在完整仿真前检查；不满足时最高可达纬度不足，直接判为不可行。
...
单重覆盖场景在本次 `standard` 模式下得到的最佳已搜索构型为 M=34、N=44、总卫星数 1496 颗，倾角 51.8036°、Walker 相位因子 F=33、Ω0=4.9826°、u0=2.7487°。其离散仿真指标为 Q1=1.000000、Q2=0.000000、P1=1.000000、P2=0.939458，平均覆盖重数为 2.9509，单重最大间隙为 0.00 min。部署需发射 25 次，总成本约 124.80 亿元。验证状态为 `validation_failed`。

二重覆盖场景在本次 `standard` 模式下得到的最佳已搜索构型为 M=40、N=46、总卫星数 1840 颗，倾角 50.3313°、Walker 相位因子 F=28、Ω0=3.5941°、u0=4.2957°。其离散仿真指标为 Q1=1.000000、Q2=0.988889、P1=1.000000、P2=0.999996，平均覆盖重数为 3.6886，单重最大间隙为 0.00 min。部署需发射 31 次，总成本约 154.00 亿元。验证状态为 `standard_search_passed_requires_full`。
